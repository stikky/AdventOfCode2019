...
void nextJump(std::string in)
{
    ...
    if (in.front() == 'R') // Right
    {
        ...
        std::cout << "(" << std::get<0>(pathA[pathA.size()-1]) << "," << std::get<1>(pathA[pathA.size()-1]) << ")>";
        // get X of last tuple and calculate new X
        int newX = std::get<0>(pathA[pathA.size()-1]) + stoi(in);
        // add tuple with new X and old Y
        pathA.push_back(i2tuple(newX, std::get<1>(pathA[pathA.size()-1])));
        // print new coords of x, y
        std::cout << "(" << std::get<0>(pathA[pathA.size()-1]) << "," << std::get<1>(pathA[pathA.size()-1]) << ")" << std::endl;
    }
    if (in.front() == 'U') // Up
    {
        std::cout << in << "|";
        ...
        int newY =std::get<1>(pathA[pathA.size()-1]) + stoi(in);
        // add tuple with old X and new Y
        pathA.push_back(i2tuple(std::get<0>(pathA[pathA.size()-1]), newY));
        // print new coords of x, y
        std::cout << "(" << std::get<0>(pathA[pathA.size()-1]) << "," << std::get<1>(pathA[pathA.size()-1]) << ")" << std::endl;  
    }
    if (in.front() == 'L') // Left
