...
void nextJump(std::string in, std::vector<i3tuple> &path, bool isVerbose=false)
{
    ...
    if (in.front() == 'U') // Up
    {
        ...
        in.front() = '0'; 
        if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << "," << std::get<2>(path[path.size()-1]) << ")>";
        // get Y of last tuple and calculate new Y
        int oldY = std::get<1>(path[path.size()-1]);
        int newY = std::get<1>(path[path.size()-1]) + stoi(in);
        // add all steps between old and new Y
        for ( int i=oldY+1; i <= newY; i++)
        {
            // add tuple with old X and new Y and increment the path length
            path.push_back(i3tuple(std::get<0>(path[path.size()-1]), i, std::get<2>(path[path.size()-1])+=1 ));
            // print new coords of x, y
            if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << "," << std::get<2>(path[path.size()-1]) << ")>";
        }
        std::cout << std::endl;
...
int main ()
{
    // initialize and calculate both paths
    std::cout << "Wire 1" << std::endl;
    wireA.push_back(i3tuple(0, 0, 0));
    for(std::string s: a)
    {
        nextJump(s, wireA, true);
    }
    std::cout << "Wire 2" << std::endl;
    wireB.push_back(i3tuple(0, 0, 0));
    for(std::string s: b)
    {
        nextJump(s, wireB, true);
    }
    std::cout << std::endl;
    std::cout << "Length of wire 1: " << wireA.size() << ", Length of wire 2: " << wireB.size() << std::endl;


    // test for wires crossing
    int lowestDistance=313373;
    int absDist=0;
    int combinedSteps=0;
    int fastestPath=313373;
    for(i3tuple t: wireA)
    {
        for(i3tuple u: wireB)
        {
            // std::cout << std::get<0>(t) << "," << std::get<1>(t) << ":" << std::get<0>(u) << "," << std::get<1>(u);
            if(std::get<0>(t) == std::get<0>(u) && std::get<1>(t) == std::get<1>(u))
            {
                absDist = abs(std::get<0>(u))+abs(std::get<1>(u));
                if ( absDist < lowestDistance && absDist != 0)
                {
                    lowestDistance = absDist;
                }
                combinedSteps = std::get<2>(t)-1 + std::get<2>(u)-1;
                if ( combinedSteps < fastestPath && combinedSteps != 0) fastestPath = combinedSteps;
                std::cout << "Match: " << std::get<0>(t) << "=" << std::get<0>(u) << ", " << std::get<1>(t) << "=" << std::get<1>(u) 
                            << "  Distance: " << absDist 
                            << "  Combined Steps: " << combinedSteps
                            << std::endl;
            }
        }
    }
    std::cout << "Lowest distance: " << lowestDistance << std::endl;
    std::cout << "fastest path: " << fastestPath << std::endl;
