...
// int pathA[sizeof(a)+1][sizeof(a)+1];
// int pathB[sizeof(b)+1][sizeof(b)+1];
...
void nextJump(std::string in, std::vector<i2tuple> &path, bool isVerbose=false)
{
    ...
    if (in.front() == 'R') // Right
    {
        ...
        for ( int i=oldX+1; oldX <= newX; i++ )
        {
            std::cout << "oldX: " << oldX << ", " << "newX: " << newX << "     ";
            // add tuple with new X and old Y
            path.push_back(i2tuple(i, std::get<1>(path[path.size()-1])));
            // print new coords of x, y
            // if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")" << std::endl;
        }
    ...
    if (in.front() == 'U') // Up
    {
        ...
        path.push_back(i2tuple(std::get<0>(path[path.size()-1]), newY));
        // print new coords of x, y
        if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")" << std::endl;  
    }
    if (in.front() == 'L') // Left
    {
        ...
        if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")>";
        int newX = std::get<0>(path[path.size()-1]) - stoi(in);
        path.push_back(i2tuple(newX, std::get<1>(path[path.size()-1])));
        if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")" << std::endl;
    }
    if (in.front() == 'D') // Down
    {
...
int main ()
{
    ...
    // std::cout << std::get<0>(pathA[0]) << std::endl;
    ...
    //         {
    //             std::cout << "Match: " << std::get<0>(t) << "=" << std::get<0>(u) << ", " << std::get<1>(t) << "=" << std::get<1>(u) << std::endl;
    //         }
    //     }
    // }
}
