...
std::vector <i2tuple> pathA;
std::vector <i2tuple> pathB;

void nextJump(std::string in, std::vector<i2tuple> &path, bool isVerbose=false)
{
    ...
    if (in.front() == 'R') // Right
    {
        ...
        if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")>";
        ...
        {
            // add tuple with new X and old Y
            path.push_back(i2tuple(i, std::get<1>(path[path.size()-1])));
    ...
    if (in.front() == 'U') // Up
    {
        ...
        {
            // add tuple with old X and new Y
            path.push_back(i2tuple(std::get<0>(path[path.size()-1]), i));
            // print new coords of x, y
            if(isVerbose) std::cout << "(" << std::get<0>(path[path.size()-1]) << "," << std::get<1>(path[path.size()-1]) << ")>";
        }
        std::cout << std::endl;
...
int main ()
{
    ...
    pathA.push_back(i2tuple(0, 0));
    for(std::string s: a)
    {
        nextJump(s, pathA, true);
    }
    std::cout << "Wire 2" << std::endl;
    pathB.push_back(i2tuple(0, 0));
    for(std::string s: b)
    {
        nextJump(s, pathB, true);
    }
    std::cout << std::endl;
    std::cout << "Length of wire 1: " << pathA.size() << ", Length of wire 2: " << pathA.size() << std::endl;
    ...
    for(i2tuple t: pathA)
    {
        for(i2tuple u: pathB)
        {
            ...
            {
                if ( abs(std::get<0>(u))+abs(std::get<1>(u)) < lowestDistance && abs(std::get<0>(u))+abs(std::get<1>(u)) != 0)
                {
                    lowestDistance = abs(std::get<0>(u))+abs(std::get<1>(u));
                }
                std::cout << "Match: " << std::get<0>(t) << "=" << std::get<0>(u) << ", " << std::get<1>(t) << "=" << std::get<1>(u) << "  Distance: " << abs(std::get<0>(u))+abs(std::get<1>(u)) << std::endl;
            }
        }
    }
